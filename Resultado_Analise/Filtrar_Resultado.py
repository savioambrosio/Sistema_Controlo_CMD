...
def Pesquisar_resultado():
    ...
    def filtrar_por_nome(tip,cli,resul,data,caminho_arquivo):
        ...
        resultados = []
        for item in dados:
            if (item.get('Tipo_Analise') == tipo or tip is None) or (item.get('Cliente') == cli or cli is None) or(item.get('Data') == data or data is None)or(item.get('Resultado') == resul or resul is None):
                        resultados.append(item)
        return resultados

    # Caminho do arquivo JSON
    caminho_arquivo = 'Projecto_Python/Ficheiros_json/Inserir_Resultados.json'

    # Nome a ser filtrado
    estilo()
    print("********** Pesquisar Resultado de Analise **********")
    estilo() 
    print("1)Pesquisar por Tipo_analise")
    print("2)Pesquisar Cliente")
    print("3)Pesquisar por Resultado")
    ...
    try:
        ...
        if (op==3):
            resul = input("Digite o Resultado a ser filtrado: ")
        if (op==4):
