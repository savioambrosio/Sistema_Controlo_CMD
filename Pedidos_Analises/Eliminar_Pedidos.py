...
def Eliminar_pedido():
    while True:
        ...
        def eliminar():
            ...
            comparar = [item for item in dados if item.get('Cliente') == valor]
            ...
            if not comparar:
                ...
                for cliente in comparar:
                    print(f" * Código: {cliente['Codigo']}, Cliente: {cliente['Cliente']}")
                linhas()
                if len(comparar) >=2:
                    chave = 'Codigo'
                    estilo()
