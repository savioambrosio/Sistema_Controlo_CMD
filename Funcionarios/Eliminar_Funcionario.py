    ...
    while True:
        ...
        def eliminar():
            ...
            if not comparar:
                ...
                print(f"Encontrados {len(comparar)}Funcionario com o nome '{valor}':")
                linhas()
                
                for cliente in comparar:
                    print(f" * Código: {cliente['Codigo']}, Departamento: {cliente['Departamento']}")
                linhas()
                if len(comparar) >=2:
                    chave = 'Codigo'
                    estilo()
                    valor = int(input(f"Qual Funcionario {valor} Desejas eliminar por Código: "))
                    linhas()
